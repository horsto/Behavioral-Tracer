...
ap.add_argument("-s", "--skipped",
	help="How many frames should be skipped?")

args = vars(ap.parse_args())

if not args.get("skipped", False):
	skipped = 1
else:
...
for video_no in xrange(len(args["video"].split(','))):
	...
	print str(skipped) + " frames should be skipped. Total frames in video: " + str(frame_count) 
	frames_analyzed = np.arange(0,frame_count,skipped)
	...
	# keep looping
	for idx_frame in tqdm(frames_analyzed[:-1]):

		frame_zaehler = frame_zaehler + 1
		camera.set(1,idx_frame+1)
		...
		if len(cnts) > 0:
			...
			if radius > 10:
	   		...
	   		cv2.putText(frame, 'x: {0:.{2}f}, y: {1:.{2}f}'.format(center[0],center[1],1),
		...
		cv2.putText(frame, "Elapsed time[s]: {0:.{2}f}, fps: {1:.{2}f}".format(time_difference,idx_frame/time_difference,1),
			(10, frame.shape[0] - 25), cv2.FONT_HERSHEY_PLAIN,
			1, (255, 255, 255), 1)	

		# if frame_zaehler%10 == 0:
		#     sys.stdout.write("\r" + "Analyzing frame " + str(idx_frame) + "/" + str(frames_analyzed[-1]) + "  " + \
		# 		'({0:.{1}f}%)'.format((float(idx_frame)/float(frames_analyzed[-1]))*100,1))
		#     sys.stdout.flush()
		...
		cv2.imshow(args["video"].split(',')[video_no] + " Frame", frame)
	...
	# finished
	print "  "
	print "Elapsed time: " + str(time_difference) + "s, FPS at the end: " + str(idx_frame/time_difference)
	...
	cv2.imwrite(args["video"].split(',')[video_no] + "_trace.png",big)
	print "Saved .png trace to: " + args["video"].split(',')[video_no] + "_trace.png"
	...
	# Save pts
	with open(args["video"].split(',')[video_no] + "_" + date_print + ".csv","a+") as saved_pts:
	    for line in pts:
	        #print line
	        coord = line
	        saved_pts.write("%s\n" %coord)
	...
	#close file again
	saved_pts.close()
	print "Saved .csv with trace info to: " + args["video"].split(',')[video_no] + ".csv"
	# delete variables
	del pts
	del time
	del saved_pts
